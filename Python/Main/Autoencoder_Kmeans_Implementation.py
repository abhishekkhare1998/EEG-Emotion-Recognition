...
def create_labels(label_dataframe):
    ...
    :param label_dataframe: label_dataframe, input Valence, Arousal data on the scale 0-10
...
if "win" in platform:
    ...
    dataset_folder_path = os.path.join(current_path.rsplit("\\", 1)[0],
                                       "Data",
                                       "Extracted_features")
else:
    current_path = os.path.realpath(__file__).rsplit("/", 1)[0]
    dataset_folder_path = os.path.join(current_path.rsplit(r"/", 1)[0],
                                       "Data",
                                       "Extracted_features")
...
for i in range_values:
    ...
    k_means.fit(amigos_data_scaled)
    score_1.append(k_means.inertia_)
...
labels = k_means.fit_predict(amigos_data_scaled)
...
for i in range_values:
    k_means = KMeans(n_clusters = i)
    k_means.fit(compressed_val)
    score_val.append(k_means.inertia_)

score_arsl = []
range_values = range(1, 20)
for i in range_values:
    k_means = KMeans(n_clusters = i)
    k_means.fit(compressed_arsl)
    score_arsl.append(k_means.inertia_)

k_means = KMeans(3)
k_means.fit(compressed_val)
kmeans_pred_labels_val = k_means.labels_

k_means = KMeans(3)
k_means.fit(compressed_arsl)
kmeans_pred_labels_arsl = k_means.labels_
